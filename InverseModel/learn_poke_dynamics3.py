...
# take forward consistency into account
...
class InverseModel(nn.Module):
	def __init__(self,
		...
		self.forward_dynamics = nn.Sequential(
			nn.Linear(feature_dim+output_dim, fwd_hidden_dim_1),
			nn.ReLU(True),
	...
	def forward(self, img1, img2, act):
		phi_img1 = self.encoder(img1)
		phi_img2 = self.encoder(img2)
		act_hat = self.final_layers(torch.cat((phi_img1, phi_img2),1))
		phi_img2_hat = self.forward_dynamics(torch.cat((phi_img1, act_hat),1))
		phi_img2_tilde = self.forward_dynamics(torch.cat((phi_img1, act), 1))
		return act_hat, phi_img2_hat, phi_img2_tilde, phi_img2

	def inverse(self,img1,img2):
		phi_img1 = self.encoder(img1)
		phi_img2 = self.encoder(img2)
		act_hat = self.final_layers(torch.cat((phi_img1, phi_img2),1))
		return act_hat 
...
class TrainInverseModel:
	def __init__(self):
		...
		self.python_script_name = 'learn_poke_dynamics3'
		feature_dim = 100
		self.jnt_dim = 4
	...
	def train(self):
		...
		writer = SummaryWriter('runs/run'+str(seed))
		dataiter = iter(training_generator)
		input_next = dataiter.next()
		images1 = input_next['img1'][:50].cuda()
		images2 = input_next['img2'][:50].cuda()
		action = input_next['act'][:50].cuda()
		img_grid = utils.make_grid(images1)
		writer.add_image('pokes', img_grid)
		writer.add_graph(self.model, (images1, images2, action))
		writer.close()
		...
		for epoch in range(max_epochs):
			...
			for sample_batch in training_generator:
				...
				# forward
				act_hat, phi_img2_hat, phi_img2_tilde, phi_img2 = self.model(img1_batch,img2_batch, act_batch)
				# import pdb; pdb.set_trace()
				loss_inverse = mse_loss(act_hat, act_batch)
				loss_forward = mse_loss(phi_img2_hat, phi_img2) + mse_loss(phi_img2_tilde, phi_img2)
				...
				running_losses['train_forward_loss'] += loss_forward.item() * img1_batch.size(0)
				# write to tensorboard
				if i%10 == 0:
					writer.add_scalar('training_inverse_loss', loss_inverse.item(), epoch*len(training_generator) + i) 
					writer.add_scalar('training_forward_loss', loss_forward.item(), epoch*len(training_generator) + i)
				i += 1
			...
			for sample_batch in valid_generator:
				...
				running_losses['val_forward_loss'] += loss_forward.item() * img1_batch.size(0)
				# write to tensorboard
				writer.add_scalar('valid_inverse_loss', loss_inverse.item(), epoch*len(valid_generator) + i)
				writer.add_scalar('valid_forward_loss', loss_forward.item(), epoch*len(valid_generator) + i)
				i += 1
	...
	def test(self, load_model=1,
		...
		np.savetxt(self.save_path + '/pd_' + data_file_str + '_lpd3.txt', save_data)
		np.savetxt(self.save_path + '/true_' + data_file_str + '_lpd3.txt', save_true_data)
		# fout.close()
...
if __name__ == '__main__':
	...
	## test
	# a = TrainInverseModel()
	# a.test(model_path='learn_poke_dynamics3_data/exp_2020-03-05-00-06-16',model_number='model_5.pth')