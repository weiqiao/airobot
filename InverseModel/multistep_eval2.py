...
from learn_poke_dynamics2 import *
...
class EvalPoke():
	...
	def eval_poke(self):
		...
		for i in range(6):
			...
			act = self.model(small_img, self.final_img_transformed)
...
if __name__ == '__main__':
	...
	if first_stage:
		...
		act_list,rel_dist_err = a.eval_poke()
		np.save('act_list2_dir/act_list_1_100_' + str(index), act_list)
		np.save('rel_dist2_err_dir/rel_dist_err_1_100_' + str(index), rel_dist_err)
		print(act_list)	
	else:	
		act_list = np.load('act_list2_dir/act_list_1_100_' + str(index) + '.npy')
		b = EvalPoke(index=index,
			ifRender = True,
			model_path=model_path,
			model_number=model_number)
		rel_dist_err = b.animation(act_list)
		np.save('rel_dist2_err_dir_render/rel_dist_err_1_100_' + str(index), rel_dist_err)
